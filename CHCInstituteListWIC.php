...
define("_ID_HC_INSTITUTE_LIST"     		, "2");


// all file inclusions are here
require_once ( "CHCInstituteListWI.php");
require_once ( "CHCInstituteListM.php");
require_once ( "CHCInstituteListStruct.php");
...
class CHCInstituteListWIC
{

    //======== member variables =========
    private $vDbConn;                                       // < database connection
    private $vCHCInstituteListMObj;                      				// < instance of the model
    private $vCHCInstituteListWIObj;			                        // < instance of the view
	private $vCHCInstituteListStructObj;		                        // < instance of the data structure
    ...
    public function __destruct() 
    {
        ...
        if($this->vDestructObj!=1){
			...
			// destroy objects
			unset( $this->vCHCInstituteListMObj  		);
			unset( $this->vCHCInstituteListWIObj   	);
			unset( $this->vCHCInstituteListStructObj  );
    ...
    public function Initialize( $pDebugTraceFlag = 0 )
    {
		...
		$this->vCHCInstituteListStructObj 	= new CHCInstituteListStruct();
        $this->vCHCInstituteListMObj			= new CHCInstituteListM();
        $this->vCHCInstituteListMObj->Initialize( $this->vDbConn, $this->vDebugTraceFlag,$this->vCHCInstituteListStructObj );
        $this->vCHCInstituteListWIObj 		= new CHCInstituteListWI();
        $this->vCHCInstituteListWIObj->Initialize( $this->vDebugTraceFlag,$this->vDbConn,$this->vCHCInstituteListStructObj); 
    ...
    private function vGetRequestVars()
    {
		...
		if(!empty($_REQUEST['ListType'])){
			$this->vEventID		= $_REQUEST['ListType'];
		}
    ...
    private function vHandleEvents()
    {
        ...
        if( $this->vDebugTraceFlag == 1 ) tlibphp_debug_trace_func ( __METHOD__."<BR>\n" );
            ...
            case _ID_HC_HOME_INVOKE_DEFAULT :
                ...
                return $this->vCHCInstituteListDefaultEventHandler();
				
			case _ID_HC_INSTITUTE_LIST :
                               
                return $this->vCHCInstituteListEventHandler();
	 ...
	 * Pending : 
     */
    private function vCHCInstituteListDefaultEventHandler()
    {
        // debug trace on function entry
        if( $this->vDebugTraceFlag == 1 ) tlibphp_debug_trace_func ( __METHOD__."<BR>\n" );

            //$this->vCHCInstituteListMObj->HCInstituteListData();            
            return $this->vCHCInstituteListWIObj->RenderView();

    }
	private function vCHCInstituteListEventHandler()
    {
        // debug trace on function entry
        if( $this->vDebugTraceFlag == 1 ) tlibphp_debug_trace_func ( __METHOD__."<BR>\n" );

            $this->vCHCInstituteListMObj->HCInstituteListData();            
            echo json_encode($this->vCHCInstituteListStructObj->vInstituteList);
    ...
    $controllerObj = new CHCInstituteListWIC();
    ...
    {
        $viewObj = new CHCInstituteListWI();
